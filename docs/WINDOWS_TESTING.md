...
The report's `coverage` fields leave visual appearance, audio playback, player-input behavior and feel, multiplayer, and other devices marked `not_verified`. The runner never generates keyboard input. Record those observations with the manual Proving Ground checklist.
...
While Minecraft is closed, bootstrap enables `content_log_file`. It backs up the original `options.txt` under `dist/bedrock-tests/setup/<id>/`, changes only that setting's value, and preserves other preferences. Bedrock 1.26.51 can omit a disabled key, so the runner adds the known setting after backup when absent; an existing malformed value still blocks setup.

Existing configured worlds are reused without replacing terrain or world settings. Generated worlds carry source and ownership markers. Conflicting destinations, ownership, and unrelated active packs are rejected. Deployment refreshes only the runner's three pack directories and activation lists. The dedicated-world copies receive deterministic test-only pack UUIDs so an older globally installed development pack with the release UUID cannot shadow current repository content; distributable manifests are not changed.
...
This project's `showcase` mode creates one truck for every color in the Sixteen-Color Palette, verifies explicit color precedence, properties, components, and seats, then checks bare/default and randomized spawn semantics. It hides the HUD and cycles three camera angles. The runner captures frames every 15 seconds into `screenshots/<frame>/minecraft.png`, observes for at least `observe_seconds`, and requires two successful captures. Showcase trucks remain in the dedicated world and are removed by their test-only tag before the next display is created. With `showcase` disabled, the ordinary smoke entity is cleaned up before PASS. Inspect captured frames before selecting images for the README; a saved frame alone is not a visual assertion.
...
The current production and harness packs request stable `@minecraft/server` 2.10.0 so cancellable before-hurt and cross-device player-button events are mandatory. See Microsoft's [Script API versioning contract](https://learn.microsoft.com/en-us/minecraft/creator/documents/scripting/versioning?view=minecraft-bedrock-stable) and the [2.10.0 versioned type definitions](https://registry.npmjs.org/@minecraft/server/-/server-2.10.0.tgz). The historical 2026-09-07 pass used 2.0.0 and therefore does not qualify the current mechanics. Other client installations require their own live validation.
...
The add-on harness tests 1,000 maximum health, a real melee hit reduced from 40 to 10 damage, ignored fall damage, fire/lava hazard damage, no damage to a nearby pig while parked, and a mob killed through the scripted Speed-Scaled Tire Trample path. Static checks guard the Suspension Jump state machine and cooldown, landing-only Crush Stomp transition, protected-target exclusions, damage scaling, spawn sources, retrieval/Scrap ownership, and projectile/explosion reduction settings. Driver Seat Spacebar input, rider retention, visible clearance, and subjective driving feel remain manual Proving Ground checks.
