...
def test_rideable_and_control_components():
    ...
    # Native Mount control model (Spacebar = Jump, Left Shift = Dismount)
    ...
    assert "minecraft:behavior.player_ride_tamed" in comps, "Entity must use player_ride_tamed for mount steering"
    assert "minecraft:input_ground_controlled" not in comps, "Must not use input_ground_controlled which forces spacebar dismount"
